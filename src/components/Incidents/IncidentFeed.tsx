...
export const IncidentFeed: React.FC<IncidentFeedProps> = ({
  ...
  return (
    <div className="flex flex-col h-full min-h-0 bg-[#0d1322] border border-slate-800/80 rounded-2xl overflow-hidden shadow-xl">
      {/* Feed Header */}
      <div className="p-2.5 sm:p-3 border-b border-slate-800 bg-[#0b0f19]/80 space-y-2 shrink-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <h3 className="font-heading font-black text-sm sm:text-base text-white">Live Incident Feed</h3>
            <span className="text-xs font-mono font-extrabold bg-slate-800 text-slate-200 px-2.5 py-0.5 rounded-full border border-slate-700">
          ...
          <div className="flex items-center gap-2">
            {onOpenCreateModal && (
              <button
                ...
                className="text-xs font-extrabold text-emerald-300 bg-emerald-950/90 hover:bg-emerald-900 px-2.5 py-1.5 rounded-lg border border-emerald-500/40 flex items-center gap-1.5 transition-all shadow-sm"
            ...
            <button
              ...
              className="text-xs font-bold text-slate-300 hover:text-white flex items-center gap-1 bg-slate-900 px-2.5 py-1.5 rounded-lg border border-slate-800 transition-colors"
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-1.5 bg-slate-900 border border-slate-800 rounded-lg text-xs sm:text-sm text-white placeholder-slate-400 focus:outline-none focus:border-red-500 transition-colors font-medium"
          />
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5 text-xs">
          <select
            ...
            className="bg-slate-900 border border-slate-800 rounded-lg px-2 py-1.5 text-slate-200 focus:outline-none focus:border-red-500 text-xs font-bold"
          >
      ...
      {/* Incident Feed List */}
      <div className="flex-1 min-h-0 overflow-y-auto p-2.5 sm:p-3 space-y-2.5 custom-scrollbar">
        {filtered.length === 0 ? (
          <div className="py-12 text-center space-y-2">
            <AlertTriangle className="w-8 h-8 text-slate-500 mx-auto" />
            <p className="text-sm text-slate-400">No active incidents match your filter criteria.</p>
            <button onClick={handleClearFilters} className="text-sm font-bold text-cyan-400 hover:underline">
              Reset Filters
            </button>
          </div>
        ) : (
          filtered.map((inc) => {
            ...
            return (
              <div
                ...
                className={`p-3 sm:p-3.5 rounded-xl border transition-all cursor-pointer select-none relative ${
                  isSelected
